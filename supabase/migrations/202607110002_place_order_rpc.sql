...
declare
  ...
  v_address text := concat_ws(', ', nullif(trim(p_address),''), nullif(trim(p_apartment),''), nullif(trim(p_city),''), nullif(trim(p_postal_code),''));
begin
  ...
  on conflict(country_id,phone) do update set full_name=excluded.full_name,email=excluded.email,updated_at=now()
  returning id into v_customer_id;
  v_order_number := 'HB-' || to_char(now(),'YYYYMMDD') || '-' || upper(substr(encode(gen_random_bytes(5),'hex'),1,5));
  insert into public.orders(order_number,idempotency_key,country_id,customer_id,customer_name,customer_phone,customer_email,currency_code,shipping_zone_id,shipping_area_name,address_line,notes,subtotal,shipping_fee,total,payment_method,status)
  values(v_order_number,p_idempotency_key,p_country_id,v_customer_id,v_full_name,p_phone,nullif(p_email,''),v_country.currency_code,
    ...
    case when v_country.use_shipping_zones then v_zone.name else p_city end,v_address,nullif(p_notes,''),v_subtotal,v_shipping,v_subtotal+v_shipping,'COD','pending')
