        ...
        <>
          ...
          <section className="rounded-lg border border-zinc-200 bg-white p-4">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <h2 className="text-lg font-semibold">Wakilni</h2>
                <p className="mt-1 text-sm text-zinc-500">
                  Sync: {typedOrder.wakilni_sync_status ?? "not_submitted"}
                  {typedOrder.wakilni_status ? ` · ${typedOrder.wakilni_status}` : ""}
                </p>
                {typedOrder.wakilni_tracking_id ? (
                  <p className="mt-1 text-sm">Tracking: {typedOrder.wakilni_tracking_id}</p>
                ) : null}
                {typedOrder.wakilni_last_error ? (
                  <p className="mt-2 max-w-3xl text-sm text-red-700">{typedOrder.wakilni_last_error}</p>
                ) : null}
              </div>
              {typedOrder.wakilni_tracking_url ? (
                <a
                  className="rounded-md border border-zinc-300 px-4 py-2 text-sm font-semibold"
                  href={typedOrder.wakilni_tracking_url}
                  rel="noreferrer"
                  target="_blank"
                >
                  Open tracking
                ...
                <form action={retryWakilniOrder.bind(null, id)}>
                  <button
